...
function SuggestionCard({ suggestion }: { suggestion: MarketSuggestion }) {
  return (
    <Link
      href={`/dashboard/market/${encodeURIComponent(suggestion.symbol)}`}
      className="group flex flex-col rounded-lg border border-border bg-card p-4 transition-all hover:border-blue-500/50 hover:shadow-md hover:shadow-blue-500/5"
    >
      {/* Header */}
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[10px] text-muted-foreground">{suggestion.name}</div>
          </div>
        </div>
...
export function AISuggestions({ tier }: AISuggestionsProps) {
  ...
  const maxCount = tier === "premium" ? 5 : 3;
    ...
    <section>
      ...
      {!collapsed && (
        <>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-center gap-2 rounded-lg border border-blue-500/20 bg-blue-500/5 py-2.5 px-4">
                ...
                <span className="text-xs font-medium text-blue-600 dark:text-blue-400">
                  Loading realtime market data via AI screening…
                </span>
              </div>
          ...
          {canAccess && !loading && !error && suggestions.length > 0 && (
            <>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
                {suggestions.map((s) => (
